...
function NewParty() {
  ...
  return (
    <div>
      ...
      {character[0] && (
        character.map((index) => {
          return <li key={index.id} onClick={() => addToParty(index)}>{index.character_name}</li>;
        }))}
      
      <form>
        <label>Name Your Party</label>
        <input
          onChange={() => setPartyMember(partyMembers => ({ ...partyMembers, party_name: event.target.value }))}
